...
class TrackingService (MessageServer):
    ...
    def process_prey_location(self,step:Step)->Step:
        # add Trajectory(JsonList)
        agent_name_in = step.agent_name
        step.agent_name = "predator"
        # print(f"processing data: Received from {agent_name_in}, sending to {step.agent_name}")
        return step

    def send_step(self, step: Step):
        # print("here")
        step = self.process_prey_location(step=step)
